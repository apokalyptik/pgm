...
type PokestopPoints struct {
	sync.RWMutex
	Points     map[string][]Point
	LastUpdate time.Time
}

func (s *PokestopPoints) Add(lat, lon float64, id string) {
	...
	if _, ok := s.Points[hash]; !ok {
		...
		if _, ok := s.Points[hash]; !ok {
			...
			s.Unlock()
			log.Printf("Added new pokestop and geohash: %s: %s: %f,%f", hash, id, lat, lon)
			return
		}
		s.Unlock()
		s.RLock()
	...
	s.LastUpdate = time.Now()
	s.Unlock()
	log.Printf("Added new pokestop point to geohash: %s: %s: %f,%f", hash, id, lat, lon)
}
